...
# La variable 'trocea' determina el tamaño del grafo a entrenar. 
# Ej: 0.3--> El número de nodos de tipo 'observación' del grafo serán 
#    el 30% de las muestras del dataset.
...
if pruebas in ['housing','concrete','power','kin8nn','aleatorio','lineal','uniforme']:
    ...
    for nB in range(numBatches):
        print("\nEntrenamiento con ",nB+1," Batches")

        for nv in range(5):
            listaBatches=np.random.choice(range(numBatches), nB+1, replace=False)    
            ...
            for i in listaBatches:
                print("-->Batch:",i, "/", nB+1 , "----Tam:", len(batch[i]), "----Tam. ultmo batch:",muestras%len(batch[i]))
            ...
            val_grape.append([nB+1,nv+1, mae, rmse, mse, accuracy])

    pdtemp=pd.DataFrame(val_grape, 
                            columns=['numBatches','exp','mae','rmse','mse','accuracy'])
    pdtemp.index.name='entrenamiento'
    d.__setitem__(pruebas, pdtemp) 
    resultados=pd.concat(d)
    resultados.to_pickle(rutaResultados+'resultados_caracterizacion.pkl')
