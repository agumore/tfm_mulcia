...
#######
# Carga modelo grape clases e intenta imputar valores perdidos del resto de los 8 no vistos.
...
if pruebas=='MNIST':
    ...
    modeloGNN.load_state_dict(torch.load(rutaResultados+rutaModelos+"modeloGNNgrapeBasicoClasif_MNIST.pt",map_location=torch.device('cpu') ))
    impute_model.load_state_dict(torch.load(rutaResultados+rutaModelos+"imputemodelgrapeBasicoClasif_MNIST.pt",map_location=torch.device('cpu') ))
    modeloGNN.eval()
    impute_model.eval()

    print("imputamos")
    prediccionNaN = imputaValores(data1, modeloGNN, impute_model, device, tipoProblema, clases, normalizaAtrib)


    #print(prediccionNaN1)
    mse,rmse,mae,accuracy  =testRed(data1.test_labels.to(device), prediccionNaN, tipoProblema)
   
    
    df_Xr=reconstruyeMatrizIncompleta(data1.df_Xi, prediccionNaN.cpu())
    print("guardando")
    guardaDataset(df_Xr,"clas_prediccion_nuevos",maxClases,rutaResultados)
