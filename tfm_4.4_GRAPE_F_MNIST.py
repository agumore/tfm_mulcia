...
#######
# En este experimento entrenamos GRAPE--> como problema de clasificación y por fases
# Entrenaremos en dos fases. 
# - Una fase "Gruesa" donde asignamos la clase mayoritaria.
# - Una fase "Fina" donde asignamos las clases NO mayoritarias.
...
if pruebas=='MNIST':
    ...
    if tipoProblema == 'regresion':
        ...
        funcPerdida = nn.CrossEntropyLoss()
    ...
    print("Entrenamiento Fase 1 con ",epochsFase1," epochs")
    ...
    for i in range(numBatches):
        ...
        modeloGNN,inpute_model,opt, best_loss= entrenaRed(data, modeloGNN, impute_model, epochsFase1, known, device, opt, normalizaAtrib, tipoProblema, funcPerdida, clases, best_loss) #pesosClases/pesosClases.sum()
    time_fin=time()
    ...
    prediccionNaNFase1 = imputaValores(data1, modeloGNN, impute_model, device, tipoProblema, clases, normalizaAtrib)
    mse,rmse,mae,accuracy  =testRed(data1.test_labels, prediccionNaNFase1, tipoProblema)
    val_baseline.append([1,mae, rmse, mse, accuracy])
    print("Tiempo total : ",time_fin-time_in)
    df_Xr=reconstruyeMatrizIncompleta(data1.df_Xi, prediccionNaNFase1)

    # Graba modelo Fase I
    torch.save(modeloGNN.state_dict(), rutaResultados+"modeloGNNgrapeFasesClasifF1_MNIST.pt")
    torch.save(impute_model.state_dict(), rutaResultados+"imputemodelgrapeFasesClasifF1_MNIST.pt")

    ############# FASE II
    
    # Obtenemos la clase mas repetida de la prediccion y un indice
    c, repc = np.unique(prediccionNaNFase1,return_counts=True)
    claseMasRepetida = c[np.argsort(-repc)][0]
    indClaseMasRepetida = prediccionNaNFase1==claseMasRepetida
    indClaseMasRepetida=np.where(indClaseMasRepetida==False,np.nan,True)
    
    #Obtenmos vector de predicciones Mayoritarias y matriz reconstruida con ellas.
    ...
    ## Pasada 2
    if sum(np.isnan(predF))!=0: # Comprobamos que no hemos asignados las mayoritarias.
        print("Entrenamiento Fase 2 con ",epochsFase2," epochs")
        best_loss=None
        for i in range(numBatches):
            print("Batch:",i+1)
            data = crea_datos(df_Xi_s2.iloc[batch[i],:], df_Xc_s.iloc[batch[i],:], df_y.iloc[batch[i],:], 0)
            modeloGNN,inpute_model,opt, best_loss= entrenaRed(data, modeloGNN, impute_model, epochsFase2, known, device, opt, normalizaAtrib, tipoProblema, funcPerdida, clases, best_loss)


        data3 = crea_datos(df_Xi_s, df_Xc_s, df_y, 0)
        prediccionNaNFase2 = imputaValores(data3, modeloGNN, impute_model, device, tipoProblema, clases, normalizaAtrib)
        #print(imputaNaN3)
        mse,rmse,mae,accuracy  = testRed(data3.test_labels, prediccionNaNFase2, tipoProblema)
        val_baseline.append([2,mae, rmse, mse, accuracy])
        ...
        df_Xr=reconstruyeMatrizIncompleta(data3.df_Xi, prediccionNaNFase2)




####################BORRAR    
    guardaDataset(df_Xr,"grapeFasesClas",maxClases,rutaResultados)
    d.__setitem__(pruebas, pd.DataFrame(val_baseline, 
                            columns=['fase','mae','rmse','mse','accuracy'], 
                            ))
    resultados=pd.concat(d)
    resultados.to_pickle(rutaResultados+'resultados_MNIST_'+"grapeFasesClas"+'.pkl')
    
    
    #Grabamos el modelo
    torch.save(modeloGNN.state_dict(), rutaResultados+"modeloGNNgrapeFasesClasifF2_MNIST.pt")
    torch.save(impute_model.state_dict(), rutaResultados+"imputemodelgrapeFasesClasifF2_MNIST.pt")
