...
#######
# Carga modelo grape regresion e intenta imputar valores perdidos de resto de 8's no vistos.
...
if pruebas=='MNIST':
    ...
    modeloGNN.load_state_dict(torch.load(rutaResultados+rutaModelos+"modeloGNNgrapeBasicoReg_MNIST.pt",map_location=torch.device('cpu') ))
    impute_model.load_state_dict(torch.load(rutaResultados+rutaModelos+"imputemodelgrapeBasicoReg_MNIST.pt",map_location=torch.device('cpu') ))
    modeloGNN.eval()
    impute_model.eval()
    ...
    guardaDataset(df_Xr,"reg_prediccion_nuevos",scaler,rutaResultados,normalizaAtrib)
