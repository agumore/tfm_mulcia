...
#######
# Carga modelo grape fases e intenta imputar valores perdidos del resto de los 8 no vistos.
...
if pruebas=='MNIST':
    ...
    modeloGNNF2 = GNNStack(dimEntNodo,    # Atributos de nodo = Dimension del embeding del nodo = Atrib. de cada observacion
    ...
    impute_modelF2 = MLPNet(input_dim, output_dim,
    ...
    modeloGNNF2.load_state_dict(torch.load(rutaResultados+rutaModelos+"modeloGNNgrapeFasesClasifF2_MNIST.pt",map_location=torch.device('cpu') ))
    impute_modelF2.load_state_dict(torch.load(rutaResultados+rutaModelos+"imputemodelgrapeFasesClasifF2_MNIST.pt",map_location=torch.device('cpu') ))
    modeloGNNF2.eval()
    impute_modelF2.eval()


    print("imputamos")
    prediccionNaNFase2 = imputaValores(data1, modeloGNNF2, impute_modelF2, device, tipoProblema, clases, normalizaAtrib)

    mse,rmse,mae,accuracy  =testRed(data1.test_labels.to(device), prediccionNaNFase2, tipoProblema)
   
    print("reconstruyendo")
    df_Xr=reconstruyeMatrizIncompleta(data1.df_Xi, prediccionNaNFase2.cpu())
    print("guardando")
    guardaDataset(df_Xr,"fases_prediccion_nuevos",maxClases,rutaResultados)
