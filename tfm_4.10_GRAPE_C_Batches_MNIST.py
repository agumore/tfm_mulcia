...
# En este experimento entrenaremos el 30% de la red por batches.
# Tendremos 4 batches y 5000 Epochs en cada uno
...
if pruebas=='MNIST':
    ...
    guardaDataset(df_Xr,"grapeBasicoClasPesos",maxClases,rutaResultados)
    d.__setitem__(pruebas, pd.DataFrame(val_baseline, 
                            columns=['mae','rmse','mse','accuracy'], 
                            ))
    resultados=pd.concat(d)
    resultados.to_pickle(rutaResultados+'resultados_MNIST_'+"grapeBasicoClasPesos"+'.pkl')
    
    
    #Grabamos el modelo
    torch.save(modeloGNN.state_dict(), rutaResultados+"modeloGNNgrapeBasicoClasifPesos_MNIST.pt")
    torch.save(impute_model.state_dict(), rutaResultados+"imputemodelgrapeBasicoClasifPesos_MNIST.pt")
